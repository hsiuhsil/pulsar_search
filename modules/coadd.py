...
def main():
    ...
    model_phase = model_phase_bin / NPHASEBIN # convert phase bins to phases
...
#    for ii in xrange(50):
    for ii in xrange(len(model_phase)):
        print 'ii: ',ii
        initial, final = np.int(bin_number[ii][0]), np.int(bin_number[ii][1])
        fold_origin = this_file['DATA_FOLDING'+'_'+str(initial)+'_'+str(final)][:, 0, :, 0].T
        profile_fft = svd.fft(fold_origin)
        phase_shift = -model_phase[ii] # minus sign for inverse the phase
        fold_trans = svd.apply_phase_shift(profile_fft, phase_shift, NPHASEBIN)
...
def plot_folding(data):
    ...
    cax1 = ax1.imshow(data, extent=[0, NPHASEBIN-1, 700., 900.],aspect='auto', cmap=cm.Greys)
