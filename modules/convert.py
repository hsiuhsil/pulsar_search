...
def main():
    ...
    psr_bs_tc_edot(period_new, pdot)
...
def psr_bs_tc_edot(period_new, pdot):
    '''Calculate strength of B field, the characteristic age, and Edot.'''
    B_s = 3.2e19 * np.sqrt(period_new * pdot) # unit: G
    t_c = 15.8 * 10**6 * period_new * (pdot/10**-15)** -1 # unit: yr
    Edot = 4 * (np.pi)**2 * 10**45 * pdot * period_new**-3 # unit erg/s
